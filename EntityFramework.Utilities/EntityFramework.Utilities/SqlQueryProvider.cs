...
namespace EntityFramework.Utilities
{
    public class SqlQueryProvider : IQueryProvider
    {
        ...
        public void InsertItems<T>(IEnumerable<T> items, string schema, string tableName, IList<ColumnMapping> properties, DbConnection storeConnection, int? batchSize, DbTransaction transaction)
        {
            using (var reader = new EFDataReader<T>(items, properties))
            {
                ...
                using (SqlBulkCopy copy = new SqlBulkCopy(con, SqlBulkCopyOptions.Default, (SqlTransaction)transaction))
                {
        ...
        public void UpdateItems<T>(IEnumerable<T> items, string schema, string tableName, IList<ColumnMapping> properties, DbConnection storeConnection, int? batchSize, UpdateSpecification<T> updateSpecification, DbTransaction transaction)
        {
            ...
            using (var createCommand = new SqlCommand(str, con, (SqlTransaction)transaction))
            using (var mCommand = new SqlCommand(mergeCommand, con, (SqlTransaction)transaction))
            using (var dCommand = new SqlCommand(string.Format("DROP table {0}.[{1}]", schema, tempTableName), con, (SqlTransaction)transaction))
            {
